...
function sp_cdm_load_languages() {
  load_plugin_textdomain( 'sp-client-document-manager', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}

function sp_cdm_loadn_textdomain( $mofile, $domain ) {
    if ( 'sp-client-document-manager' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
        $locale = apply_filters( 'plugin_locale', determine_locale(), $domain );
        $mofile = WP_PLUGIN_DIR . '/sp-client-document-manager/languages/' . $domain . '-' . $locale . '.mo';
	#echo $mofile;
    }

    return $mofile;
}
add_filter( 'load_textdomain_mofile', 'sp_cdm_loadn_textdomain', 10, 2 );
...
if ( get_option( 'sp_cu_overide_upload_path' ) != "" ) {
  ...
  define( 'SP_CDM_UPLOADS_DIR_URL', stripslashes( get_option( 'sp_cu_overide_upload_url' ) ) );
	...
	if(get_option('cdm_randomize_dir') == ''){
	$newdir_name = 'sp-client-document-manager-'. wp_generate_uuid4().'';
	$newdir = 	$cdm_upload_dir[ 'basedir' ] . '/'.$newdir_name.'/';
		if(is_dir($cdm_upload_dir[ 'basedir' ] . '/sp-client-document-manager/')){
				
			rename($cdm_upload_dir[ 'basedir' ] . '/sp-client-document-manager/', $newdir);
		}else{
			cdm_mkdir($newdir,0777);	
		}
		if(!is_dir($newdir)){
			cdm_mkdir($newdir,0777);	
		}
		update_option('cdm_randomize_dir',$newdir_name );
	}
	
  define( 'SP_CDM_UPLOADS_DIR', $cdm_upload_dir[ 'basedir' ] . '/'.get_option('cdm_randomize_dir') .'/' );
  define( 'SP_CDM_UPLOADS_DIR_URL', $cdm_upload_dir[ 'baseurl' ] . '/'.get_option('cdm_randomize_dir') .'/' );
}
...
require_once '' . dirname( __FILE__ ) . '/classes/gutenberg/index.php';
require_once '' . dirname( __FILE__ ) . '/classes/elementor/elementor-cdm-widget.php';
...
function sp_js_vars() {
  ...
  $vars = array(
    ...
    'uid' => $user_id,
	'recaptcha_enable'=>get_option('sp_cdm_recaptcha_enable'),
	'recaptcha_site_key'=>get_option('sp_cdm_recaptcha_site_key'),
...
function sp_client_upload_init() {
  ...
  if ( sp_is_used() == true ) {
    ...
    if ( get_option( 'sp_cu_jqueryui_theme' ) != 'none' ) {
      wp_register_style( 'jquery-ui-css', plugins_url( 'css/smoothness.1.12/jquery-ui.min.css', __FILE__ ) );
      wp_enqueue_style( 'jquery-ui-css' );
    }
	 	  if(get_option('sp_cdm_community_file_list_template') == 'table'){
	 wp_enqueue_script( 'data-tables', plugins_url( 'js/datatables.min.js', __FILE__ ), array(
      'jquery'
    ) );  
	 wp_enqueue_style( 'data-tables-css', plugins_url( 'css/datatables.min.css', __FILE__ ) );  
	 wp_enqueue_style( 'data-tables-css-ui', plugins_url( 'css/data-tables-jquery-ui.css', __FILE__ ) );	  
...
function sp_client_upload_menu() {
  ...
  add_menu_page( __('Documents', "sp-client-document-manager"), __('Documents', "sp-client-document-manager"), 'sp_cdm', 'sp-client-document-manager', 'sp_client_upload_options', 'dashicons-media-default', 2 );
  ...
  add_submenu_page( 'sp-client-document-manager', __('Vendors', "sp-client-document-manager"), __('Vendors', "sp-client-document-manager"), 'sp_cdm_vendors', 'sp-client-document-manager-vendors', 'sp_client_upload_options_vendors' );
  add_submenu_page( 'sp-client-document-manager', __('Help', "sp-client-document-manager"), __('Help', "sp-client-document-manager"), 'sp_cdm_help', 'sp-client-document-manager-help', 'sp_client_upload_help' );
  add_submenu_page( 'sp-client-document-manager', __('Settings', "sp-client-document-manager"),  __('Settings', "sp-client-document-manager"), 'sp_cdm_settings', 'sp-client-document-manager-settings', 'sp_client_upload_settings' );
  add_submenu_page( 'sp-client-document-manager',  __('User Files', "sp-client-document-manager"), __('User Files', "sp-client-document-manager"), 'sp_cdm_uploader', 'sp-client-document-manager-fileview', array(
    $sp_cdm_fileview,
    'view'
  ) );
...
function sp_client_upload_options() {
  ...
  if ( cdm_var( 'sp-client-document-manager-submit' ) ) {
    ...
    echo '<p style="color:green">' . __( "Updated Settings!", "sp-client-document-manager" ) . '</p>';
  }
  echo cdm_esc_html('<h2>' . __( "Latest uploads", "sp-client-document-manager" ) . '</h2>' . sp_client_upload_nav_menu() . '
