...
function sp_client_upload_settings()
{
	...
	if ( wp_verify_nonce( cdm_var('cdm_nonce_save_settings'), 'cdm_nonce_save_settings' ) == false) {exit('Security Error');}	
      ...
      if(is_array($sp_cdm_disable_features)){
	    if (count($sp_cdm_disable_features) > 0) {
            foreach ($sp_cdm_disable_features as $feature) {
                if (count($feature) > 0) {
				
                    foreach ($feature as $k=>$setting) {
                        if (isset($sp_cdm_disable_features[$k][$setting])) {
                            $sp_cdm_disable_features[$k][$setting] = 1;
                        } else {
                            $sp_cdm_disable_features[$k][$setting] = 0;
                        }
                    }
                }
            }
		...
		if (cdm_var('sp_cdm_recaptcha_enable') == "1") {
            update_option('sp_cdm_recaptcha_enable', '1');
        } else {
            update_option('sp_cdm_recaptcha_enable', '0');
        }
		
		if (cdm_var('sp_cdm_use_default_login') == "1") {
            update_option('sp_cdm_use_default_login', '1');
        } else {
            update_option('sp_cdm_use_default_login', '0');
        }
	...
	if (get_option('sp_cdm_recaptcha_enable') == 1) {
        $sp_cdm_recaptcha_enable= ' checked="checked" ';
    } else {
        $sp_cdm_recaptcha_enable = '  ';
    }
	
	if (get_option('sp_cdm_use_default_login') == 1) {
        $sp_cdm_use_default_login= ' checked="checked" ';
    } else {
        $sp_cdm_use_default_login = '  ';
    }
...
strong{font-weight:800}
  ...
  <ul>
    <li><a href="#cdm-tab-settings">' . __('Main Settings', 'sp-client-document-manager') . '</a></li>
    <li><a href="#cdm-tab-mail">' . __('Mail Settings', 'sp-client-document-manager') . '</a></li>
     <li><a href="#cdm-tab-premium">' . __('Premium Settings', 'sp-client-document-manager') . '</a></li>
	 <li><a href="#cdm-tab-advanced">' . __('Advanced Settings', 'sp-client-document-manager') . '</a></li>
	  <li><a href="#cdm-tab-disable-features">' . __('Disable Features', 'sp-client-document-manager') . '</a></li>';
...
Having problems? <a href="admin.php?page=sp-client-document-manager-settings&force_upgrade=1&force_upgrades=1">Click here to make sure your database structure is correct</a>
   ...
   <tr>
	...
	echo ''. wp_dropdown_pages( $args ).' </td>
	...
	$templates = cdm_file_list_templates();
	$default = get_option('sp_cdm_community_file_list_template', 'list');
	
	
	echo '<select name="sp_cdm_community_file_list_template">';
  ...
  echo' <tr>
    ...
    <td><input type="text" name="sp_cu_company_name"  value="' . stripslashes(esc_attr(get_option('sp_cu_company_name'))) . '"  size=80"> </td>
    ...
    <td><input type="text" name="sp_cu_filename_format"  value="' . esc_attr(get_option('sp_cu_filename_format')) . '"  size=80"><br><div style="margin:5px;padding:5px;"> Example:<br><br>
    ...
    <td><input type="text" name="sp_cu_thankyou"  value="' .strip_tags(esc_attr( get_option('sp_cu_thankyou') )). '"  size=80"> </td>
    ...
    <td><input type="text" name="sp_cu_delete"  value="' . strip_tags(esc_attr(get_option('sp_cu_delete') )). '"  size=80"> </td>

  </tr> 
      <tr>

    <td width="300"><strong>No permissions to download message</strong><br><em>Message received when a user cant download a file..</em></td>

    <td><input type="text" name="sp_cdm_permission_error_download"  value="' . strip_tags(esc_attr(get_option('sp_cdm_permission_error_download', '404: File not found') )). '"  size=80"> </td>
    ...
    <td>Singular: <input type="text" name="sp_cu_folder_name_single"   value="' . stripslashes(esc_attr(get_option('sp_cu_folder_name_single'))) . '"> Plural:  <input type="text" name="sp_cu_folder_name_plural"   value="' . stripslashes(esc_attr(get_option('sp_cu_folder_name_plural'))) . '"></td>
    ...
    <td><textarea  name="sp_cu_form_instructions"  style="width:100%;height:60px" >' . stripslashes(esc_attr(get_option('sp_cu_form_instructions'))) . '</textarea> </td>

  </tr>
    <tr>

    <td width="300"><strong>Disable login/reg form and enable default WP login</strong><br><em>Check this  if you don\'t want to use the default login/registration page and allow WP to use the default login page.</em></td>

    <td><input type="checkbox" name="sp_cdm_use_default_login"   value="1" ' . $sp_cdm_use_default_login . '> </td>

  </tr>
    <tr>

    <td width="300"><strong>Enable Recaptcha for login/reg forms?</strong><br><em>Check this to enable recaptcha.</em></td>

    <td><input type="checkbox" name="sp_cdm_recaptcha_enable"   value="1" ' . $sp_cdm_recaptcha_enable . '> </td>

  </tr>

    <tr>

    <td width="300"><strong>V3 Recaptcha Site Key</strong><br><em>Site key for recaptcha v3</em></td>

    <td><input type="text" name="sp_cdm_recaptcha_site_key"  value="' . esc_attr(get_option('sp_cdm_recaptcha_site_key') ). '" style="width:100%">  </td>

  </tr>
   
 <tr>

    <td width="300"><strong>V3 Recaptcha Secret Key</strong><br><em>Secret key for recaptcha v3</em></td>

    <td><input type="password" name="sp_cdm_recaptcha_secret_key"  value="' . esc_attr(get_option('sp_cdm_recaptcha_secret_key') ). '" style="width:100%">  </td>
  ...
  ';
    ...
    <td><input type="text" name="sp_cu_google_api_key"  value="' . esc_attr(get_option('sp_cu_google_api_key') ). '"  size=80"> </td>
...
<h2>Admin Email</h2>
 <table class="wp-list-table widefat fixed posts" cellspacing="0">
   ...
   <tr>
    ...
    <td><input style="width:100%" type="text" name="sp_cu_additional_admin_emails" value="' . stripslashes(esc_attr(get_option('sp_cu_additional_admin_emails'))) . '" ></td>
    ...
    <td>Subject: <input style="width:100%" type="text" name="sp_cu_admin_email_subject" value="' .esc_attr( get_option('sp_cu_admin_email_subject')) . '"><br>Body:<br>
		';
    echo wp_editor(stripslashes(cdm_esc_html(get_option('sp_cu_admin_email'))), 'sp_cu_admin_email');
    echo '
	...
	do_action('sp/admin/emails/under/sp_cu_admin_email');
	echo'
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
    ...
    <td><input style="width:100%" type="text" name="sp_cu_additional_user_emails" value="' . stripslashes(esc_attr(get_option('sp_cu_additional_user_emails'))) . '" ></td>
    ...
    <td>Subject: <input style="width:100%" type="text" name="sp_cu_user_email_subject" value="' . esc_attr(get_option('sp_cu_user_email_subject')) . '"><br>Body:<br>
	';
    echo wp_editor(stripslashes(cdm_esc_html(get_option('sp_cu_user_email'))), 'sp_cu_user_email');
    echo ' </td>
	...
	do_action('sp/admin/emails/under/sp_cu_user_email');
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
    ...
    <td>Subject: <input style="width:100%" type="text" name="sp_cu_admin_user_email_subject" value="' . esc_attr(get_option('sp_cu_admin_user_email_subject')) . '"><br>Body:<br>
		';
    echo wp_editor(stripslashes(cdm_esc_html(get_option('sp_cu_admin_user_email'))), 'sp_cu_admin_user_email');
    echo '
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
    ...
    <td>Subject: <input style="width:100%" type="text" name="sp_cu_vendor_email_subject" value="' . esc_attr(get_option('sp_cu_vendor_email_subject')) . '"><br>
	CC: <input style="width:100%" type="text" name="sp_cu_vendor_email_cc" value="' . esc_attr(get_option('sp_cu_vendor_email_cc') ). '"><br>
	Body:<br>
	';
    echo wp_editor(stripslashes(cdm_esc_html(get_option('sp_cu_vendor_email'))), 'sp_cu_vendor_email');
    echo '
...
<tr>
 ...
 <tr>
    ...
    <td><span style="width:120px">System Path:</span> <input type="text" name="sp_cu_overide_upload_path"  value="' . stripslashes(esc_attr(get_option('sp_cu_overide_upload_path'))) . '"  size=80"><br>

	<em><strong>Example: </strong><br>linux: /home/mysite/public_html/uploads/ <br>windows: C:\websites\mysite\uploads\</em><br><br><br>

	   <span style="width:120px"> Direct URL:</span> <input type="text" name="sp_cu_overide_upload_url"  value="' . stripslashes(esc_attr(get_option('sp_cu_overide_upload_url'))) . '"  size=80"><br>
  ...
  <tr>
    ...
    <td><input type="text" name="sp_cu_wp_folder"  value="' . stripslashes(esc_attr(get_option('sp_cu_wp_folder'))) . '"  size=80"> </td>
   ...
   <tr>
    ...
    <td><input type="text" name="sp_cu_disabled_users"  value="' . stripslashes(esc_attr(get_option('sp_cu_disabled_users'))) . '"  size=80"> </td>
 ...
 <tr>
    ...
    <td>';
	 ...
	 wp_editor(stripslashes(cdm_esc_html(get_option('sp_cu_disabled_users_error'))), 'sp_cu_disabled_users_error');
	echo ' </td>
...
<th>Disable</th>
 ...
 <input type="checkbox" name="sp_cdm_disable_features[base][cdm_disable_download]"   value="1" ' . sp_client_upload_settings_checkbox($disable_features, 'base', 'cdm_disable_download') . '></td>
 </tr>
    <tr>
