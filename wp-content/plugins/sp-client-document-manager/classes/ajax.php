...
class spdm_ajax
{
	...
	public static function extend_new_user_approve_filter($autologin){
...
public  function extend_new_user_approve_action($user){
	global $errors;
	if(class_exists('pw_new_user_approve')){
	if(get_user_meta($user->ID,'pw_user_status',true) == 'pending'){
	echo  '<div class="alert alert-info"><strong>Thanks for registering!</strong> Your account is currently pending approval.</strong></div>';		
	 ...
	 // The blogname option is escaped with esc_html on the way into the database in sanitize_option
		...
		$message = apply_filters( 'new_user_approve_request_approval_message_default', pw_new_user_approve::default_notification_message() );
		...
		// send the mail
		wp_mail( $to, $subject,apply_filters('sp_cdm/mail/content',stripslashes( $message)), pw_new_user_approve::email_message_headers() );
	}
	}
...
public function new_user_email($user, $password){
	
	$message = "Thank you for registering on ". get_bloginfo('name').". Here are your login details:
				
				Username:  ".$user->user_login."
				Password:  ".$password."
				Dashboard: ".cdm_shortcode_url('')."";
  # $headers = 'From: My Name <myname@example.com>' . "\r\n";
   wp_mail($user->user_email, 'Your registration for: '. get_bloginfo('name').'',apply_filters('sp_cdm/mail/content',stripslashes( $message)), $headers );	
	
}
	public  function remind_user_email($user, $password){
	
	$message = "You reset your password on ". get_bloginfo('name').". Here are your login details:
				
				Username:  ".$user->user_login."
				Password:  ".$password."
				Dashboard: ".cdm_shortcode_url('')."";
  # $headers = 'From: My Name <myname@example.com>' . "\r\n";
   wp_mail($user->user_email, 'Reset password: '. get_bloginfo('name').'',apply_filters('sp_cdm/mail/content',stripslashes( $message)), $headers );	
...
foreach($vars as $key=>$var){
	...
	$vars[$key] = sanitize_text_field($var);	
	}
	
	if(wp_verify_nonce( $_REQUEST['_wpnonce'], $vars['action']  ) == false){		
...
if(cdm_verify_recpatcha(array('token'=>cdm_var('token'), 'action'=>cdm_var('action'))) == false){
		echo '<div class="alert alert-danger"><strong>ERROR:</strong> Please verify you are not a bot!</div>';	
		die();
	}
	
}
if($vars['register_password1'] == '' or $vars['register_password1'] == '' or $vars['register_username'] == '' or $vars['register_email'] == ''  ){
		echo '<div class="alert alert-danger"><strong>ERROR:</strong> Please fill in all fields!</div>';	
		
}elseif( is_email($vars['register_email']) == false){
	echo '<div class="alert alert-danger"><strong>ERROR:</strong> '.esc_attr($vars['register_email']).' is not a valid email</div>';	
}else{

$username = $vars['register_username'];
$password = $vars['register_password1'];
$email = $vars['register_email'];
$user_id = username_exists( $username );

	if($vars['register_password1'] == $vars['register_password2']){
    ...
    'user_email'  => $website
				...
				$this->new_user_email($user, $password);
				
				$login['username'] = $user->user_login;
				$login['password'] = $password;		
				$autologin = 1;
				
				$autologin = apply_filters('sp_dashboard_register_login_filter',$autologin);
				do_action('sp_dashboard_register_login_action',$user);
			...
			echo '<div class="alert alert-danger"><strong>ERROR:</strong> User already exists!</div>';	
			}
	}else{		
...
public function login($vars){
	...
	if(get_option('sp_cdm_recaptcha_enable') ==1){
	$recaptcha = cdm_verify_recpatcha(array('token'=>cdm_var('token'), 'action'=>cdm_var('action')));
if($recaptcha == false){
		
		echo '<div class="alert alert-danger"><strong>ERROR:</strong> '.$recaptcha[0].'!</div>';	
		die();
	}
	...
	#print_r($vars);
	$login =  wp_authenticate($vars['username'], $vars['password']) ;
	
	if ( is_wp_error( 	$login  ) ) {
   $error_string = $login->get_error_message();
   echo '<div class="alert alert-danger"><strong>ERROR:</strong> Invalid login. <a href="#" class="reset-password">Lost your password?</a></div>';
}else{
	$user_id = $login->ID;
...
echo 'refresh';	
    ...
    function view_file($file_id = false)
    {
        ...
        $uid = $current_user->ID ;
		if(isset($_COOKIE['overide_uid']) && current_user_can('sp_cdm_admin_bar')){
			if($_COOKIE['overide_uid'] != ''){
			$uid = intval($_COOKIE['overide_uid']);
			}
		}
		...
		#if(cdm_is_file_orphan($r[0]['pid']) == true){
		#	 return '<strong>File Orphaned:</strong> Assign this file to a new folder.';
		
		#	}
      #
        if ($r[0]['recycle'] == 1) {
            return '<strong>Error:</strong> 404 File not found';
            die();
        }
        
        if (cdm_folder_permissions($r[0]['pid']) == 1 or $uid == $r[0]['uid'] or current_user_can('manage_options') == true or get_option('sp_cu_release_the_kraken') == 1 or !wp_verify_nonce( $_REQUEST['_ckey'], 'cdm-public-download'   )) {
    ...
    ';
            ...
            if (class_exists('cdmProductivityUser')) {
                $cdm_comments = new cdmProductivityUser;
...
dashicons-arrow-down-alt cdm-dashicons"></span> ' . __("Download File", "sp-client-document-manager") . '</a> ';
            
            if (sp_cdm_is_featured_disabled('base', 'cdm_disable_download') != true) {
                $html .= apply_filters('sp_cdm_viewfile_download_url', $download_url, $r);
            }
            
            
            if (cdm_user_can_delete($uid) == true && cdm_delete_permission($r[0]['pid']) == 1 && get_option('sp_cu_user_delete_disable') != 1) {
                $delete_button = '

    <a href="javascript:sp_cu_confirm_delete(\'' . get_option('sp_cu_delete') . '\',200,\'' . $r[0]['id'] . '\');" title="Delete" class="sp-cdm-delete-file" ><span class="dashicons dashicons-no cdm-dashicons"></span> ' . __("Delete File", "sp-client-document-manager") . '</a>';
            ...
            if (sp_cdm_is_featured_disabled('base', 'view_file_refresh_button') == false) {
                $html .= '<a href="#" class="cdm_refresh_file_view" data-id="' . $r[0]['id'] . '"><span class="dashicons dashicons-image-rotate cdm-dashicons"></span> ' . __('Refresh', 'sp-client-document-manager') . '</a>';
            }
...
jQuery(function($) {
    ...
    <ul>
        ...
        <li><a href="#cdm-file-main">' . __("File Info", "sp-client-document-manager") . '</a></li>';
            ...
            if (function_exists('sp_cdm_revision_add') && get_option('sp_cu_user_disable_revisions') != 1) {
                if ($r[0]['form_id'] == '' or $r[0]['form_id'] == 0) {
                    $html .= '<li><a href="#cdm-file-revisions">' . __("Revisions", "sp-client-document-manager") . '</a></li>';
                }
            }
            if (class_exists('cdmProductivityUser')) {
                if (sp_cdm_is_featured_disabled('premium', 'file_comments') == false) {
                    $html .= '<li><a href="#cdm-file-comments">' . __("Comments", "sp-client-document-manager") . '</a></li>';
                }
            }
            if (class_exists('cdmProductivityLog')) {
                if ((get_option('sp_cu_log_admin_only') == 1 && current_user_can('manage_options')) or (get_option('sp_cu_log_admin_only') == 0 or get_option('sp_cu_log_admin_only') == '')) {
                    $html .= '<li><a href="#cdm-file-log">' . __("Download Log", "sp-client-document-manager") . '</a></li>';
                }
            }
            
            if (sp_cdm_is_featured_disabled('base', 'event_logger') == false) {
                $html .= '<li><a href="#cdm-events-log">' . __("Events Log", "sp-client-document-manager") . '</a></li>';
            }
    ...
    ';
            ...
            if ($r[0]['pid'] != 0) {
                ...
                $project_title = '' . __("None", "sp-client-document-manager") . '';
            }
...
';
    ...
    <strong>' . __("File Name", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['name']) . '<br>
        ...
        <strong>' . __("File Owner", "sp-client-document-manager") . ': </strong> ' . __('You own this file', 'sp-client-document-manager') . '<br>
        ...
        <strong>' . __("File Owner", "sp-client-document-manager") . ': </strong> ' . apply_filters('sp_cdm/file/owner_name', $owner->display_name, $r[0]) . '<br>
    ...
    <strong>' . __("File Type ", "sp-client-document-manager") . ': </strong>' . $ext . '
        ...
        <strong>' . __("File Size ", "sp-client-document-manager") . ': </strong>' . _cdm_file_size($r[0]) . ' 
...
<strong>' . __("Tags ", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['tags']) . '
...
<strong>' . __("Notes ", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['notes']) . '
  ...
  ';
    ...
    function remove_cat($id = false)
    {
        ...
        if ((($current_user->ID == $r[0]['uid'] or cdmFindLockedGroup($current_user->ID, $r[0]['uid']) == true) && get_option('sp_cu_user_delete_disable') != 1) or current_user_can('manage_options') or cdm_folder_permissions($project_id) == 1) {
            ...
            #delete the project
            #sp_cdm_write_log('Recycled folder: '.$r[0]['name'].'');
            #$wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sp_cu_project WHERE id = %d",$project_id ));                            
            
            cdm_recycle('folder', $project_id);
            sp_cdm_write_log('' . $user->display_name . ' Recycled Folder: ' . $r[0]['name'] . '');
        } else {
            sp_cdm_write_log('Error: ' . $user->display_name . ' Failed recycling folder: ' . $r[0]['name'] . '');
        }
    ...
    function save_cat()
    {
        ...
        if (cdm_var('id') != "") {
            ...
            echo '' . __(sprintf("Updated %s Name", sp_cdm_folder_name()), "sp-client-document-manager") . ': ' . esc_attr($insert['name']) . '';
            cdm_delete_cache();
            exit;
        } else {
...
";
        ...
        <th class="cdm_file_info" style="text-align:left"><a href="javascript:sp_cdm_sort(\'name\',' . $jscriptpid . ')">' . __("Name", "sp-client-document-manager") . '</a></th>

        <th class="cdm_file_date"><a href="javascript:sp_cdm_sort(\'date\',' . $jscriptpid . ')">' . __("Date", "sp-client-document-manager") . '</a></th>

    

        <th class="cdm_file_type">' . __("Type", "sp-client-document-manager") . '</th>    
        ...
        <div style="padding-right:10px">';
                ...
                echo '<a href="javascript:sp_cu_dialog(\'#edit_category_' . $pid . '\',550,130)"><img src="' . SP_CDM_PLUGIN_URL . 'images/application_edit.png"> ' . __("Edit", "sp-client-document-manager") . ' ' . sp_cdm_folder_name() . ' ' . __("Name", "sp-client-document-manager") . '</a>  
    <a href="#" class="sp-cdm-delete-category" data-id="' . $pid . '" style="margin-left:20px"> <img src="' . SP_CDM_PLUGIN_URL . 'images/delete_small.png">  ' . __("Remove", "sp-client-document-manager") . ' ' . sp_cdm_folder_name() . '</a>';
        ...
        <div id="delete_category_' . $pid . '" title="' . __("Delete Category?", "sp-client-document-manager") . '">

    <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>' . __("Are you sure you would like to delete this category? Doing so will remove all files related to this category.", "sp-client-document-manager") . '</p>
            ...
            ' . sp_cdm_folder_name() . ' ' . __("Name", "sp-client-document-manager") . ': <input value="' . esc_attr(stripslashes($r_project_info[0]['name'])) . '" id="edit_project_name_' . intval($pid) . '" type="text" name="name"  style="width:200px !important"> 

            <input type="submit" value="' . __("Save", "sp-client-document-manager") . ' ' . sp_cdm_folder_name() . '" class="sp-cdm-save-category" data-id="' . $pid . '">
        ...
        if ($pid != 0) {
            ...
            echo '<td class="cdm_file_icon ext_directory" onclick="sp_cdm_load_project(' . intval($query_project[0]['parent']) . ')"></td>

        <td class="cdm_file_info" onclick="sp_cdm_load_project(' . intval($query_project[0]['parent']) . ')">&laquo; ' . __("Go Back", "sp-client-document-manager") . '</td>

        <td class="cdm_file_date" onclick="sp_cdm_load_project(' . intval($query_project[0]['parent']) . ')">&nbsp;</td>

        

        <td class="cdm_file_type" onclick="sp_cdm_load_project(' . intval($query_project[0]['parent']) . ')">' . __("Folder", "sp-client-document-manager") . '</td>    
...
';
                    ...
                    echo '
        <td class="cdm_file_icon ext_directory" onclick="sp_cdm_load_project(' . intval($r_projects[$i]['pid']) . ')"></td>

        <td class="cdm_file_info" onclick="sp_cdm_load_project(' . intval($r_projects[$i]['pid']) . ')">' . esc_attr(stripslashes($r_projects[$i]['project_name'])) . '</td>

        <td class="cdm_file_date" onclick="sp_cdm_load_project(' . intval($r_projects[$i]['pid']) . ')">&nbsp;</td>
		...
		if(!in_array($sort,array('name','date'))){
		$sort = 'name';	
   ...
   public static function folder_files($id)
    {
        global $wpdb, $current_user;
        if (!is_user_logged_in())
            exit;
    ...
    public static  function folder_name($id)
    {
        global $wpdb, $current_user;
        if (!is_user_logged_in())
            exit;
      ...
      public static function get_folder_structure($pid)
    {
    ...
    function email_vendor()
    {
        ...
        if (wp_verify_nonce(cdm_var('cdm_nonce'), 'cdm_nonce') == false) {
            exit('Security Error');
        }
        ...
        if (count(cdm_var('vendor_email')) == 0) {
            echo '<p style="color:red;font-weight:bold">' . __("Please select at least one file!", "sp-client-document-manager") . '</p>';
        } else {
            ...
            echo '<p style="color:green;font-weight:bold">' . __("Files Sent to", "sp-client-document-manager") . ' ' . cdm_var('vendor') . '</p>';
        }
    }
}
