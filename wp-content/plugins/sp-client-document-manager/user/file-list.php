    ...
    function edit_folder_navigation()
      {
       ...
       $pid = intval(cdm_var('pid'));
		$user_permissions = cdm_file_permissions( $pid);
        
		if (( $pid != "0" &&  $pid != '') && ((get_option('sp_cu_user_projects') == 1 && get_option('sp_cu_user_projects_modify') != 1) or current_user_can('manage_options') or $user_permissions == 1))
          {
            ...
            $r_project_info = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "sp_cu_project where id = " .  $pid . "  ORDER by " . $project_listing_type . "", ARRAY_A);
                ...
                if ($user_permissions == 1 && get_option('sp_cu_user_projects_modify') != 1)
                  {
                    echo '<a href="javascript:sp_cu_dialog(\'#edit_category_' . sanitize_text_field(cdm_var('pid')) . '\',550,130)"><img src="' . SP_CDM_PLUGIN_URL . 'images/application_edit.png"> ' .__(sprintf("Edit %s Name",sp_cdm_folder_name()), "sp-client-document-manager"). '</a>';
                  }
                ...
                if (cdm_delete_permission(cdm_var('pid')) == 1 && get_option('sp_cu_user_projects_modify') != 1)
                  {
                    ...
                    echo '<a href="#" class="sp-cdm-delete-category" data-id="' . sanitize_text_field(cdm_var('pid')) . '" style="margin-left:20px"> <img src="' . SP_CDM_PLUGIN_URL . 'images/delete_small.png">  ' .__(sprintf("Remove %s",sp_cdm_folder_name()), "sp-client-document-manager") . ' </a>';
					   }
                  }
		...
		<div id="delete_category_' .  $pid . '" title="' .__(sprintf("Remove %s?",sp_cdm_folder_name()), "sp-client-document-manager"). '">

	<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>' .__(sprintf("Are you sure you would like to delete this %s? Doing so will remove all files related to this %s.",sp_cdm_folder_name(),sp_cdm_folder_name()), "sp-client-document-manager")  . '</p>
			...
			<input type="hidden"  name="edit_project_id" id="edit_project_id_' .  $pid . '" value="' .  $pid . '">		

			' . sp_cdm_folder_name() . ' ' . __("Name", "sp-client-document-manager") . ': <input value="' . esc_attr(stripslashes($r_project_info[0]['name'])) . '" id="edit_project_name_' .  $pid . '" type="text" name="name"  style="width:200px !important"> 

			<input type="submit" value="' . __("Save", "sp-client-document-manager") . ' ' . sp_cdm_folder_name() . '"  class="sp-cdm-save-category" data-id="' . sanitize_text_field(cdm_var('pid')) . '" >
        ...
        if (in_array(strtolower($ext), $images_arr))
          {
            if (get_option('sp_cu_overide_upload_path') != '' && get_option('sp_cu_overide_upload_url') == '')
              ...
              {
                $img = '<img src="' .esc_attr( sp_cdm_thumbnail('' . SP_CDM_UPLOADS_DIR_URL . '' . $file['uid'] . '/' . $file['file'] . '', NULL, 70) ). '"  rel="' . esc_attr(sp_cdm_thumbnail('' . SP_CDM_UPLOADS_DIR_URL . '' . $file['uid'] . '/' . $file['file'] . '', 250)) . '" width="' . $width . '" class="cdm-hover-thumb">';
