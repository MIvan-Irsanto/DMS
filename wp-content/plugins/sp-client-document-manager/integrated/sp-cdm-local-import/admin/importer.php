<?php
class SP_CDM_RecursiveFilterIteratorExtend extends RecursiveFilterIterator {
...
ini_set('upload_max_filesize', '1024M');
	  ...
	  $this->copy_local_files($_REQUEST['path'],$_REQUEST['uid']);
...
if (!copy($file, $local_file_path)) {
    echo "failed to copy ".esc_attr($file)."...\n";
}
	#}
...
function process($pid,$uid){
	...
	echo cdm_esc_html('<div class="updated"><p>Total left: '. count($_SESSION['continue_folder_import']).'</p></div>');
...
$folder_count= 0;
	...
	$starting_path = $path_dir[$total];
		...
		$current_folder = $current_folder_dir[$current_folder_total];		
			...
			$parent = sanitize_text_field($_SESSION['structure'][$_SESSION['folder_import_original'][$parent_path]['__base_id']]);
									
			}
			
			if($current_folder != $starting_path){
			$set_pid = $this->add_folder($current_folder ,$parent,$uid);
			echo '<div class="updated"><p>Added Folder : '.esc_attr($current_folder).'</p></div>';
			$folder_count++;
	...
	echo '<div class="updated"><p>Imported '. intval($file_count).' files into '.esc_attr($current_folder).'</p></div>';
...
$folder_count= 0;
	...
	if($loop_total >= $total){
		...
		if($path ==cdm_var('path')){
			...
			$file_count++;
				...
				$parent_folder = $loop_path_dir[$loop_parent];
					...
					$parent = sanitize_text_field( $_SESSION['structure'][$parent_path.'_'.$parent_folder]);
					...
					#echo 'parent:'.$parent_path.'_'.$parent_folder.'';
						...
						$folder_count++;
						$_SESSION['structure'][esc_attr($path).'_'.esc_attr($current_folder)] = $pid;
						foreach($files as $file_key=>$file){
						if(!in_array($file,array('.','..')) && $file_key != '__base_id'){
						$this->add_file($path.'/'.$file,$pid,$uid);	
						$file_count++;
		...
		echo '<div class="updated"><p>Imported '. intval($file_count).' files and '.intval($folder_count).' folders!</p></div>';
...
function explodeTree($array, $delimiter = '_', $baseval = false)
{
	...
	add_submenu_page('sp-client-document-manager', __("Local Import", "sp-client-document-manager"), __("Local Import", "sp-client-document-manager"), 'sp_cdm', 'sp-client-document-manager-local-import', array(
        $this,
        'view'
    ));
	...
	function top_menu($html){
		...
		$html .= '<li><a href="admin.php?page=sp-client-document-manager-local-import" >' . __("Local Import", "sp-client-document-manager") . '</a></li>';
		return $html;	
	}
	function view(){
		  session_start();
	...
	if(@$_SESSION['import_starting_dir'] == ''){
		...
		$dir = sanitize_text_field($_SERVER['DOCUMENT_ROOT']) ;
	}else{
		
		$dir =sanitize_text_field( $_SESSION['import_starting_dir']);
	}
		
	 echo '<h2>' . __("Local Import", "sp-client-document-manager") . '</h2>' . sp_client_upload_nav_menu() . '
